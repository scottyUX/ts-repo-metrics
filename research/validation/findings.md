...
> # ⚠ SUPERSEDED — do not cite any number in this document
>
> **Every figure below describes `analyzer_version` 0.0.0 at engine commit
> `1f250c1e`.** The engine is now at **0.2.0**. The headline agreement rates
> further down (98.2% / 91.8% / 5.4%) are three generations out of date:
>
> | | this document (0.0.0) | **current (0.2.0)** |
> |---|---|---|
> | structural vs ts-complex | 98.2% | **99.81%** |
> | cognitive vs sonarjs | 91.8% | **92.44%** |
> | lexical vs escomplex | 5.4% | **6.49%** |
>
> **For the numbers to quote, see [`../../FINAL_NUMBERS.md`](../../FINAL_NUMBERS.md).**
> Machine-generated current values live in [`summary.json`](summary.json) and
> [`stats_tables.md`](stats_tables.md), which ARE regenerated by `analyze.py`.
> This file's prose is hand-written (`analyze.py:731`) and is therefore kept as a
> historical record of the ten divergence patterns, not as a live result.
>
> **Defect status since this was written:** D1, D3, D5, D9, D10 fixed (0.1.0).
> D7 fixed and **D4 replaced outright** (0.2.0) — D4's original rule was found not
> to generalize and was superseded by Sonar's B1/B2/B3 separation, see
> [`d4_generalization/findings_d4.md`](d4_generalization/findings_d4.md) and
> [`d7_halstead_fix/findings_d7.md`](d7_halstead_fix/findings_d7.md).
> **D2, D6, and D8 remain open.**
>
> Frozen outputs from the 0.0.0 state are in [`pre_fix_baseline/`](pre_fix_baseline/);
> the intermediate 0.1.0-era comparison is
> [`POST_FIX_COMPARISON.md`](POST_FIX_COMPARISON.md), itself now superseded.
